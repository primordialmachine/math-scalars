...
TEST(arithmetic_functors_test, binary_star_functor_test)
{
  ...
  static_assert(true == has_arity_v<binary_star_functor<float, float>>,
                "arity must be available");
  //
  static_assert(false == has_binary_star_functor_v<char, char>,
                "binary star functor must not be available");
  static_assert(false == has_result_type_v<binary_star_functor<char, char>>,
...
TEST(arithmetic_functors_test, binary_slash_functor_test)
{
  ...
  static_assert(true == has_result_type_v<binary_slash_functor<float, float>>,
                "result type must be available");
  static_assert(true == has_arity_v<binary_slash_functor<float, float>>,
                "arity must be available");
  //
  static_assert(false == has_binary_slash_functor_v<char, char>,
                "binary slash functor must not be available");
  static_assert(false == has_result_type_v<binary_slash_functor<char, char>>,
                "result type must not be available");
  static_assert(false == has_result_type_v<binary_slash_functor<char, char>>,
                "result type must not be available");
...
TEST(arithmetic_functors_test, binary_plus_functor_test)
{
  ...
  static_assert(true == has_result_type_v<binary_plus_functor<float, float>>,
                "result type must be available");
  static_assert(true == has_arity_v<binary_plus_functor<float, float>>,
                "arity must be available");
  //
  static_assert(false == has_binary_plus_functor_v<char, char>,
                "binary plus functor must not be available");
  static_assert(false == has_result_type_v<binary_plus_functor<char, char>>,
                "result type must not be available");
...
TEST(arithmetic_functors_test, binary_minus_functor_test)
{
  ...
  static_assert(true == has_result_type_v<binary_minus_functor<float, float>>,
                "result type must be available");
  static_assert(true == has_arity_v<binary_minus_functor<float, float>>,
                "arity must be available");
  //
  static_assert(false == has_binary_minus_functor_v<char, char>,
                "binary minus functor must not be available");
  static_assert(false == has_result_type_v<binary_minus_functor<char, char>>,
                "result type must not be available");
...
TEST(arithmetic_functors_test, unary_plus_functor_test)
{
  ...
  static_assert(true == has_result_type_v<unary_plus_functor<float>>,
                "result type must be available");
  static_assert(true == has_arity_v<unary_plus_functor<float>>,
                "arity must be available");
  //
  static_assert(false == has_unary_plus_functor_v<char>,
                "unary plus functor must not be available");
  static_assert(false == has_result_type_v<unary_plus_functor<char>>,
                "result type must not be available");
...
TEST(arithmetic_functors_test, unary_minus_functor_test)
{
  ...
  static_assert(true == has_result_type_v<unary_minus_functor<float>>,
                "result type must be available");
  static_assert(true == has_arity_v<unary_minus_functor<float>>,
                "arity must be available");
  //
  static_assert(false == has_unary_minus_functor_v<char>,
                "unary minus functor must not be available");
  static_assert(false == has_result_type_v<unary_minus_functor<char>>,
                "result type must not be available");
...
TEST(arithmetic_functors_test, plus_assignment_functor_test)
{
  ...
  static_assert(true ==
                  has_result_type_v<plus_assignment_functor<float, float>>,
                "result type must be available");
  static_assert(true == has_arity_v<plus_assignment_functor<float, float>>,
                "arity must be available");
  //
  static_assert(false == has_plus_assignment_functor_v<char, char>,
                "plus assignment functor must not be available");
  static_assert(false == has_result_type_v<plus_assignment_functor<char, char>>,
                "result type must not be available");
  float x = 1.f;
...
TEST(arithmetic_functors_test, minus_assignment_functor_test)
{
  ...
  static_assert(true ==
                  has_result_type_v<minus_assignment_functor<float, float>>,
                "result type must be available");
  static_assert(true == has_arity_v<minus_assignment_functor<float, float>>,
                "arity must be available");
  //
  static_assert(false == has_minus_assignment_functor_v<char, char>,
                "minus assignment functor must not be available");
  static_assert(false ==
                  has_result_type_v<minus_assignment_functor<char, char>>,
                "result type must not be available");
  float x = 1.f;
...
TEST(arithmetic_functors_test, slash_assignment_functor_test)
{
  ...
  static_assert(true ==
                  has_result_type_v<slash_assignment_functor<float, float>>,
                "result type must be available");
  static_assert(true == has_arity_v<slash_assignment_functor<float, float>>,
                "arity must be available");
  //
  static_assert(false == has_slash_assignment_functor_v<char, char>,
                "slash assignment functor must not be available");
  static_assert(false ==
                  has_result_type_v<slash_assignment_functor<char, char>>,
                "result type must not be available");
  float x = 6.f;
...
TEST(arithmetic_functors_test, square_root_functor_test)
{
  ...
  static_assert(true == has_result_type_v<square_root_functor<float>>,
                "result type must be available");
  static_assert(true == has_arity_v<square_root_functor<float>>,
                "arity must be available");
  //
  static_assert(false == has_square_root_functor_v<char>,
                "square root functor must not be available");
  static_assert(false == has_result_type_v<square_root_functor<char>>,
                "result type must not be available");
...
TEST(arithmetic_functors_test, star_assignment_functor_test)
{
  ...
  static_assert(true ==
                  has_result_type_v<star_assignment_functor<float, float>>,
                "result type must be available");
  static_assert(true == has_arity_v<star_assignment_functor<float, float>>,
                "arity must be available");
  //
  static_assert(false == has_star_assignment_functor_v<char, char>,
                "star assignment functor must not be available");
  static_assert(false == has_result_type_v<star_assignment_functor<char, char>>,
                "result type must not be available");
  float x = 2.f;
